...
public class Rule {
	...
	public Rule(RuleBase Rb, String Name,ArrayList<Clause> lhs, Clause rhs ){
		rb = Rb;
		name = Name;
		antecedents = new Clause[lhs.size()];
	...
	void display(JFXListView<String> textArea) {
		String str;
		str = name +": IF ";
		for(int i=0 ; i < antecedents.length ; i++) {
			Clause nextClause = antecedents[i] ;
			str += nextClause.lhs.name +
		...
		str += "\n THEN ";
		str += consequent.lhs.name +
		consequent.cond.asString() + consequent.rhs + "\n";

		textArea.getItems().add(str);
	}

	void display(javafx.scene.control.TextArea textArea) {
